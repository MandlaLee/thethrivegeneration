(function(){
  ...
  function draw(group='All'){grid.innerHTML=members.filter(m=>group==='All'||m.group===group).map(m=>`<article class="member-card"><img src="${m.image}" alt="Placeholder portrait for ${m.name}"><div><span>${m.group}</span><h3>${m.name}</h3><p class="member-role">${m.role}</p><p>${m.bio}</p></div></article>`).join('');}
